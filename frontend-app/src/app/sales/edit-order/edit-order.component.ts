...
export class EditOrderComponent implements OnInit {
  ...
  private title: String = "Редагування замовлення";
  ...
  ngOnInit() {
    ...
    this.sharingSalesDataService.saleCurrentData.subscribe(sale => this.sales = sale);
      this.httpActionsService.getSalesItem(this.saleId).subscribe(sal => {
        this.newOrder = sal;
        ...
        .subscribe(str => {
          ...
          .subscribe(dir => {
            ...
            this.newOrder.drugs.forEach(item => {
              let newGroup = this.fb.group({
                drugName: [item.drugName, [Validators.required]],
                drugCost: [item.drugCost, [Validators.required]],
                drugQuantity: [item.drugQuantity, [Validators.required]]
              });
  ...
  onSubmit(){
      this.httpActionsService.deleteSalesData(this.saleId).subscribe(sale => {
        ...
        for(let i = 0; i < this.myForm.value.formArray.length; i++){
          ...
          this.newOrder.finishCost = <number>this.newOrder.finishCost + (<number>this.orderDrug.drugQuantity * <number>this.orderDrug.drugCost);
